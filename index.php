...
if($user['role']==='admin' || $isViewer){
  ...
  if(!$isViewer && $page==='admin_submission_form'){
    ...
    $hierarchyLevels=[];
    if(!empty($s['hierarchy_path'])){
      $hierarchyLevels=array_filter(array_map('trim',explode('/',(string)$s['hierarchy_path'])));
      if(!empty($s['parent_submission_id'])){
        $pp=$pdo->prepare('SELECT name,hierarchy_path FROM submissions WHERE id=:id');$pp->execute(['id'=>(int)$s['parent_submission_id']]);$pv=$pp->fetch();
        if($pv){$parentParts=[]; if(!empty($pv['hierarchy_path'])){$parentParts=array_filter(array_map('trim',explode('/',(string)$pv['hierarchy_path'])));} $parentParts[]=(string)$pv['name'];
          $matches=true; foreach($parentParts as $i=>$part){if(!isset($hierarchyLevels[$i]) || $hierarchyLevels[$i]!==$part){$matches=false;break;}}
          if($matches){$hierarchyLevels=array_slice($hierarchyLevels,count($parentParts));}
        }
      }
    }
