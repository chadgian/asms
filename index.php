...
if($user['role']==='agency'){
  ...
  if($page==='notifications'){
    ...
    $st=$pdo->prepare('SELECT n.*,s.name submission_name FROM notifications n LEFT JOIN agency_submissions a ON a.id=n.agency_submission_id LEFT JOIN submissions s ON s.id=a.submission_id WHERE n.recipient_user_id=:u ORDER BY n.created_at DESC');$st->execute(['u'=>$user['id']]);$rows=$st->fetchAll(); ?>
    <section class="card"><div class="row"><h2>Notifications</h2><input data-search-target="#notif-table" placeholder="Search..."></div><table id="notif-table"><thead><tr><th>Submission</th><th>Message</th><th>Date</th></tr></thead><tbody><?php foreach($rows as $r): ?><tr><td><?=h($r['submission_name']?:'-')?></td><td><?=h($r['message'])?></td><td><?=h(format_datetime($r['created_at']))?></td></tr><?php endforeach; ?></tbody></table></section>
    <?php render_footer(); exit;
  }
  ...
  if($page==='agency_submission'){
    ...
    <section class="card submission-hero"><h2><?=h($sub['name'])?></h2><p><?=nl2br(h($sub['details']??''))?></p><p><strong>Deadline:</strong> <?=h(date('M d, Y',strtotime($sub['deadline'])))?></p><p><strong>Latest Status:</strong> <?=badge($sub['latest_status'])?></p><div class="row-wrap"><?php foreach($tpls as $tp): ?><a class="btn btn-view" href="uploads/<?=h($tp['file_path'])?>" download="<?=h($tp['file_name'])?>">Download</a><?php endforeach; ?></div></section>
    ...
    <section class="card"><h3>Uploaded Batches</h3><?php foreach($groups as $batch=>$items): $ts=$items[0]['uploaded_at']??null; ?><details open><summary><?=h(format_datetime($ts))?> (<?=count($items)?> files)</summary><table><thead><tr><th>File</th><th>Status</th><th>Remarks</th><th>Date</th><th>Actions</th></tr></thead><tbody><?php foreach($items as $x): ?><tr><td><?=h($x['file_name'])?></td><td><?=badge($x['document_status'])?></td><td><?=h($x['admin_remarks']?:($x['user_remarks']?:'-'))?></td><td><?=h(format_datetime($x['uploaded_at']))?></td><td class="actions compact"><a class="btn btn-view" href="uploads/<?=h($x['file_path'])?>" download="<?=h($x['file_name'])?>">Download</a><?php if(!empty($x['admin_remarks'])): ?><button type="button" class="btn btn-edit" onclick="alert(<?= json_encode($x['admin_remarks']) ?>)">View Remarks</button><?php else: ?><button type="button" class="btn btn-edit" disabled>No Remarks</button><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></details><?php endforeach; ?></section>
    <?php render_footer(); exit;
  }
...
if($user['role']==='admin' || $isViewer){
  ...
  if(!$isViewer && $page==='admin_update_document' && $_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($asid){$pdo->prepare('UPDATE agency_submissions SET latest_status=:s,updated_at=NOW() WHERE id=:id')->execute(['s'=>$status,'id'=>$asid]);}
