...
if($user['role']==='admin' || $isViewer){
  ...
  <?php if(!$isViewer): $agg=$pdo->query("SELECT u.id,u.name,u.province,COUNT(a.id) c FROM users u LEFT JOIN agency_submissions a ON a.agency_id=u.id WHERE u.role='agency' GROUP BY u.id,u.name,u.province ORDER BY u.name")->fetchAll(); ?>
  <section class="card"><h3>Agency Submission Summary</h3><?php foreach($agg as $g): ?>
    <details <?= ((int)$g['c']>0)?'':'open' ?>><summary><?=h($g['name'])?> (<?= (int)$g['c'] ?> submissions)</summary>
      <?php $lst=$pdo->prepare('SELECT s.name,a.latest_status,a.submitted_at FROM agency_submissions a JOIN submissions s ON s.id=a.submission_id WHERE a.agency_id=:a ORDER BY a.submitted_at DESC,s.deadline DESC');$lst->execute(['a'=>$g['id']]);$lrows=$lst->fetchAll(); ?>
      <?php if(!$lrows): ?><p class="muted">No submission yet.</p><?php else: ?><table><thead><tr><th>Submission</th><th>Status</th><th>Date</th></tr></thead><tbody><?php foreach($lrows as $i=>$lr): if($i===0): ?><tr><td><?=h($lr['name'])?></td><td><?=badge($lr['latest_status'])?></td><td><?=h(format_datetime($lr['submitted_at']))?></td></tr><?php endif; endforeach; ?></tbody></table><?php if(count($lrows)>1): ?><details><summary>Show older submissions</summary><table><tbody><?php foreach($lrows as $i=>$lr): if($i>0): ?><tr><td><?=h($lr['name'])?></td><td><?=badge($lr['latest_status'])?></td><td><?=h(format_datetime($lr['submitted_at']))?></td></tr><?php endif; endforeach; ?></tbody></table></details><?php endif; ?><?php endif; ?>
    </details>
